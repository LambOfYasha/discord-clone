...
export default function TestEventPage() {
  ...
  const [serverId, setServerId] = useState<string>(search.get("serverId") || "");
  ...
  const submit = async () => {
    ...
    if (type === EventType.VOICE && !voiceChannelId) {
      toast.error("Select a voice channel");
      return;
    }
    if (type === EventType.OTHER) {
      ...
      if (otherType === OtherEventLocationType.TEXT_CHANNEL && !textChannelId) {
        toast.error("Select a text channel");
        return;
      }
      if (otherType === OtherEventLocationType.EXTERNAL && !externalUrl) {
        toast.error("Enter an external link");
        return;
      }
      if (otherType === OtherEventLocationType.LOCATION && !location) {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-4">
      <h1 className="text-2xl font-bold">Test Event Creator</h1>
      <p className="text-sm text-zinc-500">Use this page to test event creation without the modal.</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="text-xs font-medium">Server ID</label>
          <Input value={serverId} onChange={(e) => setServerId(e.target.value)} placeholder="server-id" />
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-xs font-medium">End time (optional)</label>
      ...
      <div className="flex gap-3">
        ...
        <Button variant="outline" onClick={() => navigator.clipboard.writeText(serverId)}>Copy Server ID</Button>
      </div>
    </div>
  );
}
