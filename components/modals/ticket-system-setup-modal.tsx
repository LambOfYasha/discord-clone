...
interface TicketSystemSetupModalProps {
  server: Server;
}
...
export const TicketSystemSetupModal = () => {
  ...
  const isModalOpen = isOpen && type === "ticketSystemSetup";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Ticket system setup successfully!");
        onClose();
      } else {
        const error = await response.text();
        toast.error(error || "Failed to setup ticket system");
      }
    } catch (error) {
      console.error("Setup error:", error);
      toast.error("Failed to setup ticket system");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <DialogContent className="bg-white dark:bg-[#313338] text-black dark:text-white p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-zinc-500 dark:text-zinc-400">
            Configure the ticket support system for your server
          </DialogDescription>
        </DialogHeader>
