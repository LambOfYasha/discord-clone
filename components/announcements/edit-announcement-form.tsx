...
export const EditAnnouncementForm = ({ server, channels, member, announcement }: EditAnnouncementFormProps) => {
  ...
  useEffect(() => {
    if (announcement) {
      ...
      setChannelId(announcement.channelId);
      setScheduleType(announcement.scheduleType);
      setIsActive(announcement.isActive);

      // Extract time from schedule data
      if (announcement.scheduleData?.dailyTime) {
        setCustomTime(announcement.scheduleData.dailyTime);
      } else if (announcement.scheduleType === "CUSTOM" && announcement.scheduleData?.customDateTime) {
        // Extract date and time for custom schedules
        const date = new Date(announcement.scheduleData.customDateTime);
        setCustomDate(date.toISOString().split('T')[0]);
        setCustomTime(date.toTimeString().slice(0, 5));
      } else if (announcement.nextSendAt) {
        // Fallback: extract time from nextSendAt for any schedule type
        const nextSendDate = new Date(announcement.nextSendAt);
        setCustomTime(nextSendDate.toTimeString().slice(0, 5));
      }
    }
  }, [announcement]);
    ...
    <div className="max-w-2xl mx-auto">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Modify your scheduled announcement settings and content.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-muted p-4 rounded-lg">
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                ...
                <p>Created by: {announcement.creatorProfile.name}</p>
                {announcement.lastSentAt && (
                  <p>Last sent: {new Date(announcement.lastSentAt).toLocaleString()}</p>
                )}
                <p>Next scheduled: {new Date(announcement.nextSendAt).toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push(`/servers/${server.id}/announcements`)}
                disabled={isLoading}
              >
                Cancel
              </Button>
